...
def _merge_dicts(
    new_dict, restore_from, allow_extra=True, allow_missing=True, prefix=tuple()
):
...
def load_pretrained_params(
    pretrained_params,
    pretrained_extra_variables,
    params,
    extra_variables,
    prefix_key="encoder/encoder",
):
    """
    Args:
    ...
    params = _merge_dicts(
        params, pretrained_params, True, True, prefix_tuple
    )  # Just checking
    ...
    if not (set(pretrained_extra_variables.keys()) <= set(extra_variables.keys())):
        ...
        logging.error(
            set(pretrained_extra_variables.keys()) - set(extra_variables.keys())
        )

    for k in pretrained_extra_variables:
        extra_variables[k] = _merge_dicts(
            extra_variables[k], pretrained_extra_variables[k], True, True, prefix_tuple
        )

...
def get_fixed_pos_embed(embed_dim, length):
    ...
    pos_embed = jnp.concatenate(
        [jnp.zeros([1, 1, embed_dim], dtype=jnp.float32), pos_embed_nocls], axis=1
    )
    return pos_embed
...
class Attention(nn.Module):
    ...
    def __call__(self, inputs, deterministic=None, padding_mask=None):
        ...
        x = nn.Dense(
            self.dim, kernel_init=nn.initializers.xavier_uniform(), name="proj"
        )(x)
...
class Block(nn.Module):
    ...
    def __call__(self, inputs, deterministic=False, padding_mask=None):
        ...
        x = Attention(
            self.emb_dim, self.num_heads, True, self.att_drop, self.drop, name="attn"
        ...
        x = TransformerMLP(
            self.emb_dim * self.mlp_ratio, self.emb_dim, self.drop, name="mlp"
        )(x, deterministic)
...
class MaskedAutoencoder(nn.Module):
    ...
    def setup(self):
        ...
        self.decoder_input_projection = nn.Dense(
            self.dec_emb_dim, kernel_init=nn.initializers.xavier_uniform()
        )
    ...
    def forward_representation(self, image, train=True):
        ...
        cls_token = jnp.broadcast_to(self.cls_token, (batch_size, 1, self.emb_dim))
    ...
    def forward_encoder(self, image, deterministic=False):
        ...
        image_x = image_x + self.positional_embedding[:, 1:, :]
    ...
    def forward_decoder(self, x, image_ids_restore, deterministic=False):
        ...
        image_keep_length = int(
            image_ids_restore.shape[0] * (1.0 - self.image_mask_ratio)
        )
        ...
        image_x = image_x + get_2d_sincos_pos_embed(
            self.dec_emb_dim, image_ids_restore.shape[0]
        )
...
class MaskedAutoencoderVIT(nn.Module):
    ...
    def setup(self):
        self.patch_embed = PatchEmbed(
            patch_size=self.patch_size, embed_dim=self.emb_dim
        )
        # CLS and masks
        self.cls_token = self.param(
            "cls_token",

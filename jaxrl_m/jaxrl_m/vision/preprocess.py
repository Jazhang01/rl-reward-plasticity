...
def preprocess_observations(
    ...
    if resize:
        if (
            obs.shape[-3] != resize_shape[0] or obs.shape[-2] != resize_shape[1]
        ):  # Already resized
            if obs.shape[-3] != resize_shape[0] or obs.shape[-2] != resize_shape[1]:
                logging.info("Resizing to %s" % str(resize_shape))
                obs = jax.image.resize(
                    obs,
                    (*obs.shape[:-3], *resize_shape, obs.shape[-1]),
                    method="bilinear",
                )
...
class PreprocessEncoder(nn.Module):
    ...
    default_kwargs: dict = (
        None  # Default arguments, will be overriden by kwargs passed to __call__
    )
...
BasicPreprocessEncoder = ft.partial(
    PreprocessEncoder,
    normalize=True,
